...
if (!nexacro.Grid) {
	...
	_pGridRow.on_apply_prop_rtl = function (v) {
		nexacro.Component.prototype.on_apply_prop_rtl.call(this, this._rtl);
		var cells = this._cells;
		var _rtl = this._rtl;

		if (cells) {
			for (var i = 0, n = cells.length; i < n; i++) {
				cells[i].set_rtl(_rtl);
			}
		}
	};
	...
	_pGridBand.on_apply_prop_rtl = function (v) {
		nexacro.Component.prototype.on_apply_prop_rtl.call(this, this._rtl);
	...
	_pGrid.on_apply_prop_rtl = function (v) {
		...
		var _rtl = this._rtl;
		if (this._headBand) {
			this._headBand.set_rtl(_rtl);
		}
		if (this._bodyBand) {
			this._bodyBand.set_rtl(_rtl);
		}
		if (this._summBand) {
			this._summBand.set_rtl(_rtl);
		}
	};
