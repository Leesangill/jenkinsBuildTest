...
if (nexacro._Browser == "Runtime") {
	...
	if (!nexacro.Element) {
		...
		_pElement._getRTLPositionLeft = function (left, width) {
			...
			if (owner_elem && bPositionRtl && !(this instanceof nexacro._FrameResizeBorderElement)) {
				var parent_width = owner_elem.width;
		...
		_pControlElement.setElementSize = function (width, height, isInnerFrame) {
			...
			var handle = this.handle;
			if (handle) {
				var newLeft = this._getRTLPositionLeft(this.left, this.width);
				if (newLeft || this.top) {
					nexacro.__setElementHandlePosition(handle, newLeft, this.top);
				}
